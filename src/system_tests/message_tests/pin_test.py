'''
This module only tests for message/pin
'''
import requests
import urllib
...
from system_tests.fixtures.common_set import BASE_CHANNEL, URL_RESET, URL_ADDOWNER, \
    ...
    URL_PROFILE, URL_MSG_SEND, URL_MSG_PIN
...
def test_basic():
    ...
    # ch_id2: owner = token_1; member = token_1, token_0
    ...
    invite_user_channel(token_1, ch_id2, users_data[0]['u_id'], URL_INVITE)
    ...
    assert payload2["messages"][2]["is_pinned"] == True
    assert payload2["messages"][0]["is_pinned"] == True
...
def test_already_pinned():
    ...
    response = requests.post(URL_MSG_PIN, json={"token": token_0, "message_id": msg1_1})
...
def test_invalid_message_id():
    ...
    response = requests.post(URL_MSG_PIN, json={"token": token_0, "message_id": "NOT_VALID_ID", "message": "new_msg_1(1)"})
    assert response.status_code == 400 or response.status_code == 500
...
def test_user_not_in_channel():
    ...
    # pin some messages where you arent in the channel
    response = requests.post(URL_MSG_PIN, json={"token": token_1, "message_id": msg1_1})
    response2 = requests.post(URL_MSG_PIN, json={"token": token_2, "message_id": msg1_2})
...
def test_user_not_owner_of_slackr_or_channel():
    ...
    # pin some messages where you arent owner of channel or slackr
    response = requests.post(URL_MSG_PIN, json={"token": token_2, "message_id": msg1_1})
    assert response.status_code == 400